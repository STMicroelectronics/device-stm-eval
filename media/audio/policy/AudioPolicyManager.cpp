...
non_direct_output:
    ...
    if (audio_is_linear_pcm(format)) {
        ...
        // could not find previously an output supporting requested device(s)
        // check if devices can be managed through multiple outputs
        if ((output == 0) && (mOutputs.size()>1)) {
            sp<AudioOutputDescriptor> desc;
            sp<AudioOutputDescriptor> dupOutputDesc;
            audio_io_handle_t output1 = 0;
            audio_io_handle_t output2 = 0;
            audio_io_handle_t duplicatedOutput = AUDIO_IO_HANDLE_NONE;
            size_t max = 2*mOutputs.size()-1;

            for (size_t i = 0; i < mOutputs.size(); i++) {

                if ((mOutputs.valueAt(i)->isDuplicated()) && (i == max)) {
                    break;
                }

                if (!output1) {
                    output1 = mOutputs.keyAt(i);
                } else {
                    output2 = mOutputs.keyAt(i);
                }

                if (output1 && output2){
                    // open a duplicating output thread for output1 and output2
                    duplicatedOutput = mpClientInterface->openDuplicateOutput(output1, output2);

                    if (duplicatedOutput != AUDIO_IO_HANDLE_NONE) {
                        // add duplicated output descriptor
                        desc = mOutputs.valueAt(i);
                        dupOutputDesc = new AudioOutputDescriptor(NULL);
                        dupOutputDesc->mOutput1 = mOutputs.valueFor(output1);
                        dupOutputDesc->mOutput2 = mOutputs.valueFor(output2);
                        ...
                        ALOGI("duplicated output %d added for %d and %d",
                              duplicatedOutput, mPrimaryOutput, mOutputs.keyAt(i));
                    } else {
                        ALOGW("openDuplicateOutput() failed for output %d and %d",
                              output1, output2);
                        output = 0;
                        break;
                    }
    ...
    mBeaconMuted(false)
{
    ...
    ALOGI("Enter AudioPolicyManager");
                                                             ...
                                                             bool fromCache)
{
    ...
    case STRATEGY_MEDIA: {
        ...
        if (strategy == STRATEGY_MEDIA) {
            // ARC, SPDIF and AUX_LINE and SPEAKER can co-exist with others.
            ...
            device3 |= (availableOutputDeviceTypes & AUDIO_DEVICE_OUT_SPEAKER);
        }
